...
#+BEGIN_SRC emacs-lisp
  (defvar hb-diminish-package-mode-alist
  ...
  (defvar hb-diminish-packages
    (cl-remove-if 'not
                  (append (mapcar 'car hb-diminish-package-mode-alist)
                          '(diminish))))

  (defun hb-diminish/emit-post-init-defun-name (package)
    (intern (format "hb-diminish/post-init-%s" package)))

  (defvar hb-diminish-post-init-function-mode-file-list
    (mapcar (lambda (x)
              (list (hb-diminish/emit-post-init-defun-name (car x)) (car x) (cdr x)))
            (cl-remove-if (lambda (x) (not (car x))) hb-diminish-package-mode-alist)))

  (defun hb-diminish/post-init-diminish ()
    (mapcar
     (lambda (mode)
       (eval `(spacemacs|diminish ,mode)))
     (mapcar 'cdr (cl-remove-if 'car hb-diminish-package-mode-alist))))

  (defun hb-diminish/emit-post-init-defun (defun-name mode file)
    `(defun ,defun-name ()
       (eval-after-load ,file
         '(progn (message "diminishing %s from %s" ',mode ',file)
                 (spacemacs|diminish ,mode)))))

  (defmacro hb-diminish/emit-post-init-defuns ()
    `(progn
       ,@(mapcar
          (lambda (package-mode-file)
            (let ((defun-name (car package-mode-file)) (file (symbol-name (cadr package-mode-file))) (mode (nth 2 package-mode-file)))
              (hb-diminish/emit-post-init-defun defun-name mode file)))
          hb-diminish-post-init-function-mode-file-list)))

  (hb-diminish/emit-post-init-defuns)
#+END_SRC
