...
#+BEGIN_SRC emacs-lisp
  (setq hb-c++-packages '(function-args))
...
** embrace
:PROPERTIES:
:header-args+: :tangle layers/hb-embrace/packages.el
:END:

#+BEGIN_SRC emacs-lisp
  (setq hb-embrace-packages '(embrace))

  (defun hb-embrace/init-embrace ()
    (use-package embrace))

  (defun hb-embrace/post-init-embrace ()
...
This looks cool, I need to investigate sometime.
