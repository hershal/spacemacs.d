...
#+BEGIN_SRC emacs-lisp
  (defvar hb-diminish-packages
    '(diminish aggressive-indent company evil-org smartparens holy-mode
               hybrid-mode yasnippet which-key))
  (defvar hb-diminish-additional-packages '(auto-fill-mode))
  (defvar package-mode-alist '((yasnippet . yas-minor-mode)))

  (defun hb-diminish/post-init-diminish ()
    (let ((diminish-modes
           (mapcar '(lambda (m) (cons m (mode-from-package m)))
                   (union hb-diminish-additional-packages
                          (set-difference hb-diminish-packages '(diminish))))))
